...
if [ "$DRY_RUN" = true ]; then
    ...
    echo "DRY RUN: Would restart CADT service"
else
    ...
    # Restart the CADT service
    echo "Restarting CADT service..."
    SERVICE_NAME="cadt@${CHIA_USER}.service"
    if systemctl is-active --quiet "$SERVICE_NAME"; then
        systemctl restart "$SERVICE_NAME"
        echo "CADT service restarted successfully"
    else
        echo "Warning: CADT service is not running. Skipping restart."
    fi
fi
