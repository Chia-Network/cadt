...
if (nodeEnv === 'test') {
  ...
  if (testConfig.storage.includes('~') || testConfig.storage.includes(os.homedir())) {
    const errorMsg = `SAFETY CHECK FAILED: Test database path appears to be in home directory: ${testConfig.storage}. Test databases must be in project root (relative paths like './test.sqlite3').`;
    console.error(errorMsg);
    throw new Error(errorMsg);
  }
}
