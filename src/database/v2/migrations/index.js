import CreateProjectV2 from './20250109120000-create-project-v2.js';
import CreateValidationV2 from './20250109120001-create-validation-v2.js';
import CreateVerificationV2 from './20250109120002-create-verification-v2.js';
import CreateMethodologyV2 from './20250109120003-create-methodology-v2.js';
import CreateLocationV2 from './20250109120004-create-location-v2.js';
import CreateIssuanceV2 from './20250109120005-create-issuance-v2.js';
import CreateUnitV2 from './20250109120006-create-unit-v2.js';
import CreateProjectMethodologyV2 from './20250109120007-create-project-methodology-v2.js';
import CreateStakeholderV2 from './20250109120008-create-stakeholder-v2.js';
import CreateStakeholderProjectsV2 from './20250109120009-create-stakeholder-projects-v2.js';
import CreateLabelV2 from './20250109120010-create-label-v2.js';
import CreateUnitLabelV2 from './20250109120011-create-unit-label-v2.js';
import CreateCoBenefitV2 from './20250109120012-create-co-benefit-v2.js';
import CreateEstimationV2 from './20250109120013-create-estimation-v2.js';
import CreateRatingV2 from './20250109120014-create-rating-v2.js';
import CreateProgramV2 from './20250109120015-create-program-v2.js';
import CreateAefT1SubmissionV2 from './20250109120016-create-aef-t1-submission-v2.js';
import CreateAefT2AuthorizationsV2 from './20250109120017-create-aef-t2-authorizations-v2.js';
import CreateAefT3ActionsV2 from './20250109120018-create-aef-t3-actions-v2.js';
import CreateAefT4HoldingsV2 from './20250109120019-create-aef-t4-holdings-v2.js';
import CreateAefT5AuthorizedEntitiesV2 from './20250109120020-create-aef-t5-authorized-entities-v2.js';
import CreateStagingV2 from './20250110120000-create-staging-v2.js';
import CreateAuditV2 from './20250110120001-create-audit-v2.js';
import CreateOrganizationsV2 from './20250110120002-create-organizations-v2.js';
import CreateMetaV2 from './20250110120003-create-meta-v2.js';
import CreateGovernanceV2 from './20250110120004-create-governance-v2.js';
import CreateSimulatorV2 from './20250110120005-create-simulator-v2.js';

export const migrations = [
  {
    migration: CreateProjectV2,
    name: '20250109120000-create-project-v2.js',
  },
  {
    migration: CreateValidationV2,
    name: '20250109120001-create-validation-v2.js',
  },
  {
    migration: CreateVerificationV2,
    name: '20250109120002-create-verification-v2.js',
  },
  {
    migration: CreateMethodologyV2,
    name: '20250109120003-create-methodology-v2.js',
  },
  {
    migration: CreateLocationV2,
    name: '20250109120004-create-location-v2.js',
  },
  {
    migration: CreateIssuanceV2,
    name: '20250109120005-create-issuance-v2.js',
  },
  {
    migration: CreateUnitV2,
    name: '20250109120006-create-unit-v2.js',
  },
  {
    migration: CreateProjectMethodologyV2,
    name: '20250109120007-create-project-methodology-v2.js',
  },
  {
    migration: CreateStakeholderV2,
    name: '20250109120008-create-stakeholder-v2.js',
  },
  {
    migration: CreateStakeholderProjectsV2,
    name: '20250109120009-create-stakeholder-projects-v2.js',
  },
  {
    migration: CreateLabelV2,
    name: '20250109120010-create-label-v2.js',
  },
  {
    migration: CreateUnitLabelV2,
    name: '20250109120011-create-unit-label-v2.js',
  },
  {
    migration: CreateCoBenefitV2,
    name: '20250109120012-create-co-benefit-v2.js',
  },
  {
    migration: CreateEstimationV2,
    name: '20250109120013-create-estimation-v2.js',
  },
  {
    migration: CreateRatingV2,
    name: '20250109120014-create-rating-v2.js',
  },
  {
    migration: CreateProgramV2,
    name: '20250109120015-create-program-v2.js',
  },
  {
    migration: CreateAefT1SubmissionV2,
    name: '20250109120016-create-aef-t1-submission-v2.js',
  },
  {
    migration: CreateAefT2AuthorizationsV2,
    name: '20250109120017-create-aef-t2-authorizations-v2.js',
  },
  {
    migration: CreateAefT3ActionsV2,
    name: '20250109120018-create-aef-t3-actions-v2.js',
  },
  {
    migration: CreateAefT4HoldingsV2,
    name: '20250109120019-create-aef-t4-holdings-v2.js',
  },
  {
    migration: CreateAefT5AuthorizedEntitiesV2,
    name: '20250109120020-create-aef-t5-authorized-entities-v2.js',
  },
  {
    migration: CreateStagingV2,
    name: '20250110120000-create-staging-v2.js',
  },
  {
    migration: CreateAuditV2,
    name: '20250110120001-create-audit-v2.js',
  },
  {
    migration: CreateOrganizationsV2,
    name: '20250110120002-create-organizations-v2.js',
  },
  {
    migration: CreateMetaV2,
    name: '20250110120003-create-meta-v2.js',
  },
  {
    migration: CreateGovernanceV2,
    name: '20250110120004-create-governance-v2.js',
  },
  {
    migration: CreateSimulatorV2,
    name: '20250110120005-create-simulator-v2.js',
  },
];
