...
export const unitV2Schema = Joi.object({
  ...
  unitCount: Joi.number().min(0).required(),
  unitType: Joi.string().required().custom(pickListValidationV2('unitType', 'Unit Type')),
  unitVintageYear: Joi.number().integer().min(1900).max(2100).required(),
  unitStatus: Joi.string().required().custom(pickListValidationV2('unitStatus', 'Unit Status')),
  unitStatusReason: Joi.string().required(),
  ...
  unitLink: Joi.string()
    ...
    .allow(null, '')
    .optional(),
  unitMetric: Joi.string().required().custom(pickListValidationV2('unitMetric', 'Unit Metric')),
