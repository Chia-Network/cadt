...
const getDataLayerStoreSyncStatus = async (storeId) => {
  ...
  try {
    ...
    if (Object.keys(data).includes('success')) {
      ...
      logger.warn(
        `datalayer '/get_sync_status' RPC failed to get sync status for ${storeId}: response missing 'success' key`,
      );
    }
    ...
    logger.error(
      `getDataLayerStoreSyncStatus: error for storeId ${storeId}: ${error.message}`,
    );
    return false;
  }
};
