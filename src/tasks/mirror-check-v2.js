...
const task = new Task('mirror-check-v2', async () => {
  logger.silly('[v2]: [MIRROR_DEBUG] Mirror-check V2 task started');

  try {
    logger.silly('[v2]: [MIRROR_DEBUG] Checking data layer availability');
    await assertDataLayerAvailable();
    logger.silly('[v2]: [MIRROR_DEBUG] Data layer is available');

    logger.silly('[v2]: [MIRROR_DEBUG] Checking wallet sync status');
    await assertWalletIsSynced();
    logger.silly('[v2]: [MIRROR_DEBUG] Wallet is synced');
    ...
    if (!APP_CONFIG.USE_SIMULATOR && shouldMirror) {
      logger.silly('[v2]: [MIRROR_DEBUG] Conditions met, running mirror check');
      await runMirrorCheckV2();
    } else {
      logger.silly('[v2]: [MIRROR_DEBUG] Skipping mirror check - conditions not met');
    }
  } catch (error) {
    logger.error(
    ...
    logger.silly(`[v2]: [MIRROR_DEBUG] Mirror-check V2 task error: ${error.message}`);
  }
});
...
const runMirrorCheckV2 = async () => {
  logger.silly('[v2]: [MIRROR_DEBUG] Starting runMirrorCheckV2 function');

  const mirrorUrl = await getMirrorUrl();
  logger.silly(`[v2]: [MIRROR_DEBUG] Retrieved mirror URL: ${mirrorUrl}`);

  if (!mirrorUrl) {
    logger.info(
      '[v2]: DATALAYER_FILE_SERVER_URL not set, skipping mirror announcements',
    );
    logger.silly('[v2]: [MIRROR_DEBUG] Exiting runMirrorCheckV2 - no mirror URL');
    return;
  }

  // get governance info if governance node
  logger.silly('[v2]: [MIRROR_DEBUG] Checking for V2 governance organization info');
  ...
  if (
    ...
    logger.silly('[v2]: [MIRROR_DEBUG] Adding governance mirrors');
    ...
    logger.silly('[v2]: [MIRROR_DEBUG] Completed governance mirror additions');
  } else {
    logger.debug(
  ...
  logger.silly('[v2]: [MIRROR_DEBUG] Retrieving V2 organizations map');
  ...
  logger.silly(`[v2]: [MIRROR_DEBUG] Processing ${orgs.length} organizations`);

  for (const org of orgs) {
    logger.silly(`[v2]: [MIRROR_DEBUG] Processing organization: ${org}`);
  ...
  logger.silly('[v2]: [MIRROR_DEBUG] Completed runMirrorCheckV2 function');
};
