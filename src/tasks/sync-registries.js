...
const syncOrganizationAudit = async (organization) => {
  ...
  try {
    ...
    logger.debug(
      `1 Last processed index of ${organization.name}: ${auditTableHighestProcessedGenerationIndex}`,
    );
    ...
    if (!CONFIG.USE_SIMULATOR) {
      ...
      if (toBeProcessedDatalayerGenerationIndex + 1 > rootHistory.length) {
        const warningMsg = [
          `Generation ${toBeProcessedDatalayerGenerationIndex + 1} does not exist in ${organization.name} (store ${organization.orgUid}) root history`,
