...
When you successfully run CADT, it will create the unified config file at `~/.chia/mainnet/cadt/config.yaml`. The configuration file has three sections: `APP` (shared configuration), `V1` (V1-specific settings), and `V2` (V2-specific settings). The configuration file options are documented at [https://github.com/Chia-Network/cadt#configuration](https://github.com/Chia-Network/cadt#configuration).
