...
Running this application will create a new folder in your CHIA_ROOT directory (usually the .chia folder in your home directory). This folder will store the unified configuration file for the CADT instance, along with the data for the CADT instance. The unified config file can be found at `C:\Users<user>\chia\mainnet\cadt\config.yaml` on Windows or `~/.chia/mainnet/cadt/config.yaml` on Linux and Mac (where "~" is a shortcut to your home directory). The config file has three sections: `APP` (shared configuration), `V1` (V1-specific settings), and `V2` (V2-specific settings). Database files are stored in `cadt/v1` and `cadt/v2` subdirectories.
...
_If you intend to set up a "staging" or "test" environment, you will need to configure your CADT application to point to testnet. This will require you to open the unified config.yaml file, which can be found at `~/.chia/mainnet/cadt/config.yaml`._
