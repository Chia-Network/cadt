...
describe('Unit Resource Integration Tests', function () {
  ...
  it(
    'deletes a unit end-to-end (with simulator)',
    async function () {
      /*
  ...
  it(
    'splits an existing unit end-to-end (with simulator)',
    async function () {
      // create and commit the unit to be deleted
      ...
      const createdCommitResult = await supertest(app).post(
        '/v1/staging/commit',
      );
      ...
      const unitRes = await supertest(app)
        .post('/v1/units/split')
        .send(payload);
  ...
  it(
    'creates a new unit end-to-end  (with simulator)',
    async function () {
      // 1. Create a new unit
  ...
  it(
    'updates a new unit end-to-end (with simulator)',
    async function () {
      // create and commit the unit to be deleted
      const newUnitPayload = await testFixtures.createNewUnit();
