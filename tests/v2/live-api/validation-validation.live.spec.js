...
describe('Validation Live API Validation Tests', function () {
  ...
  describe('Step 3: Validation Failure Tests', function () {
    ...
    it('should reject POST with invalid foreign keys', async function () {
      const invalidData = generateValidationMinimal(getNonExistentId());
      const response = await request
        .post('/v2/validation')
        .send(invalidData);
  ...
  describe('Step 8: GET Request Tests', function () {
    ...
    it('should filter validations by orgUid=me', async function () {
      const response = await request
        .get('/v2/validation?orgUid=me&page=1&limit=10')
        .expect(200);
