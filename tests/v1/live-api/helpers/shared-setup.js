...
export async function runSharedSetup(skipEmptyCheck = false) {
  ...
  try {
    ...
    // Also skip if SKIP_EMPTY_CHECK env var is set (for testing with existing data)
    if (!skipEmptyCheck && process.env.SKIP_EMPTY_CHECK !== 'true') {
      try {
        await checkDatabaseEmpty(sharedRequest);
      } catch (error) {
        // If database is not empty, log warning but don't fail (for testing with existing data)
        console.warn(`⚠️  ${error.message}`);
        console.warn('⚠️  Continuing tests with existing data in database');
      }
    } else {
      console.log('✓ Skipping empty database check');
    }
